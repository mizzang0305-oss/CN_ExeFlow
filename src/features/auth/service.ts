...
async function loadDepartmentsMap(client: ReturnType<typeof createSupabaseServerClient>, departmentIds: Array<string | null>) {
  const ids = Array.from(new Set(departmentIds.filter((departmentId): departmentId is string => Boolean(departmentId))));
  ...
  if (error) {
    throw new ApiError(500, "부서 목록을 불러오지 못했습니다.", error, "SESSION_DEPARTMENTS_LOAD_FAILED");
  }
...
export async function listImpersonationUsersAsSession(session: AppSession) {
  ...
  if (error) {
    throw new ApiError(500, "사용자 목록을 불러오지 못했습니다.", error, "IMPERSONATION_USERS_LOAD_FAILED");
  }

  const departmentsMap = await loadDepartmentsMap(client, (data ?? []).map((user) => user.department_id));

  return (data ?? []).map((user) => ({
    departmentName: departmentsMap.get(user.department_id ?? "")?.name ?? null,
    displayName: buildDisplayName(user),
    email: normalizeEmailAddress(user.email),
    id: user.id,
    role: user.role,
