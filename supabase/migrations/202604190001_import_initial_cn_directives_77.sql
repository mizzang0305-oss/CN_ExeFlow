...
-- 3. 검증이 끝나면 COMMIT, 점검만 필요하면 ROLLBACK 기준으로 운용합니다.
--
-- 설계 메모:
-- - Supabase SQL Editor / migration 실행 환경 차이로 temp/staging relation 의존성을 제거했습니다.
-- - 모든 원본 행은 DO 블록 내부에서 source line -> record 로만 변환합니다.
...
do $$
declare
  v_import_year int := 2025;
  v_import_batch text := 'CN_INITIAL_IMPORT_77';
  v_all_department_owner_name text := '주식회사 씨엔푸드';
  v_source_lines text := $data$
...
$data$;
  ...
  v_all_department_owner_id uuid;
  v_active_department_count int;
  v_processed_count int := 0;
  v_expected_assignment_count int := 0;
  v_seen_directive_nos text[] := array[]::text[];
  v_status_en text;
  v_target_scope text;
  v_directive_no text;
  v_year_month text;
  ...
  v_owner_department_name text;
  v_existing_directive_id uuid;
  v_existing_is_import boolean;
  v_directive_id uuid;
  ...
  v_note text;
  v_target_department_ids uuid[];
  v_target_department_names text[];
  v_target_department_head_ids uuid[];
  r record;
  dep_legacy text;
  dep_row record;
  i int;
begin
  ...
  if v_import_user_id is null then
    raise exception 'CN import blocked: no active CEO or SUPER_ADMIN user found for created_by.';
  end if;

  select d.id
  into v_all_department_owner_id
  from public.departments d
  where d.is_active = true
    and d.name = v_all_department_owner_name
  limit 1;

  if v_all_department_owner_id is null then
    raise exception
      'CN import blocked: active owner department for ALL scope not found: %.',
      v_all_department_owner_name;
  end if;

  select count(*)
  into v_active_department_count
  from public.departments d
  where d.is_active = true;

  if v_active_department_count = 0 then
    raise exception 'CN import blocked: no active departments found.';
  end if;

  for r in
    with raw_lines as (
      select btrim(line, E'\r') as line
      from regexp_split_to_table(v_source_lines, E'\n') as line
  ...
  loop
    ...
    if r.status_ko <> '진행중' then
      raise exception
        'CN import blocked: unsupported source status "%" at source_no %.',
        r.status_ko,
        r.source_no;
    end if;

    v_status_en := 'IN_PROGRESS';
    v_directive_no := format(
      'CN-%s-%s-%s',
      v_import_year,
      lpad(r.meeting_month::text, 2, '0'),
      lpad(r.month_seq::text, 3, '0')
    );
    v_year_month := format('%s-%s', v_import_year, lpad(r.meeting_month::text, 2, '0'));
    v_created_at := make_timestamptz(v_import_year, r.meeting_month, r.meeting_day, 9, 0, 0, 'Asia/Seoul');
    v_target_department_ids := array[]::uuid[];
    v_target_department_names := array[]::text[];
    v_target_department_head_ids := array[]::uuid[];
    v_owner_department_id := null;
    v_owner_department_name := null;

    if v_directive_no = any(v_seen_directive_nos) then
      raise exception
        'CN import blocked: duplicate directive_no generated: %.',
        v_directive_no;
    end if;

    v_seen_directive_nos := array_append(v_seen_directive_nos, v_directive_no);

    if r.departments_raw = '전 부서' then
      v_target_scope := 'ALL';
      v_owner_department_id := v_all_department_owner_id;
      v_owner_department_name := v_all_department_owner_name;
      ...
      for dep_row in
        ...
        where d.is_active = true
        order by
          case when d.id = v_all_department_owner_id then 0 else 1 end,
          d.sort_order nulls last,
          d.name
      loop
        v_target_department_ids := array_append(v_target_department_ids, dep_row.id);
        v_target_department_names := array_append(v_target_department_names, dep_row.name);
        v_target_department_head_ids := array_append(v_target_department_head_ids, dep_row.head_user_id);
      end loop;
    else
      v_target_scope := 'SELECTED';

      foreach dep_legacy in array regexp_split_to_array(r.departments_raw, '\s*,\s*')
      loop
        v_canonical_name := case trim(dep_legacy)
          when '기획영업부' then '영업본부'
          when '경영지원센터' then '경영관리부'
          when 'HACCP' then '햅썹운용팀'
          when '구매물류부' then '물류부'
          when '육가공' then '육가공팀'
          else null
        end;
        ...
        if v_canonical_name is null then
          raise exception
            'CN import blocked: unmapped legacy department "%" at source_no %.',
            trim(dep_legacy),
            r.source_no;
        end if;
        ...
        from public.departments d
        where d.is_active = true
          and d.name = v_canonical_name
        order by d.sort_order nulls last, d.name
        limit 1;

        if v_department_id is null then
          raise exception
            'CN import blocked: mapped active department "%" not found for source_no %.',
            v_canonical_name,
            r.source_no;
        end if;

        if v_department_id = any(v_target_department_ids) then
          continue;
        end if;

        v_target_department_ids := array_append(v_target_department_ids, v_department_id);
        v_target_department_names := array_append(v_target_department_names, v_department_name);
        v_target_department_head_ids := array_append(v_target_department_head_ids, v_department_head_id);

        if v_owner_department_id is null then
          v_owner_department_id := v_department_id;
          v_owner_department_name := v_department_name;
        end if;
      end loop;
    end if;

    if coalesce(array_length(v_target_department_ids, 1), 0) = 0 then
      raise exception
        'CN import blocked: no target departments prepared for source_no %.',
        r.source_no;
    end if;

    if v_owner_department_id is null or v_owner_department_name is null then
      raise exception
        'CN import blocked: owner department resolution failed for source_no %.',
        r.source_no;
    end if;

    v_content := r.directive_text
      || E'\n\n[원본이관정보]'
      || E'\n- 이관배치: ' || v_import_batch
      || E'\n- 원본번호: ' || r.source_no::text
      || E'\n- 원본회의일: ' || r.meeting_date_raw
      || E'\n- 적용연도: ' || v_import_year::text
      || E'\n- 주관: ' || r.chair_role
      || E'\n- 원본상태: ' || r.status_ko
      || E'\n- 원본담당부서: ' || r.departments_raw
      || E'\n- 이관주관부서: ' || v_owner_department_name
      || E'\n- 이관담당부서: '
      || case
        when v_target_scope = 'ALL'
          then '전 부서 -> 현재 활성 부서 전체 배정 (' || v_active_department_count::text || '개)'
        else array_to_string(v_target_department_names, ', ')
      end
      || E'\n- source_type(보존): MEETING'
      || E'\n- priority(보존): MEDIUM'
      || coalesce(E'\n- 비고: ' || v_note, '');

    v_existing_directive_id := null;
    v_existing_is_import := null;

    select
      d.id,
      position(v_import_batch in coalesce(d.content, '')) > 0
    into
      v_existing_directive_id,
      v_existing_is_import
    from public.directives d
    where d.directive_no = v_directive_no
    limit 1;

    if v_existing_directive_id is not null and coalesce(v_existing_is_import, false) = false then
      raise exception
        'CN import blocked: directive_no collision with non-import row: %.',
        v_directive_no;
    end if;
    ...
    if v_existing_directive_id is not null then
      ...
      set
        title = r.directive_text,
        content = v_content,
        year_month = v_year_month,
        sequence = r.month_seq,
        owner_department_id = v_owner_department_id,
        target_scope = v_target_scope
      where d.id = v_existing_directive_id;
    ...
    for i in 1..coalesce(array_length(v_target_department_ids, 1), 0)
    loop
      v_department_id := v_target_department_ids[i];
      v_department_head_id := v_target_department_head_ids[i];
      v_is_primary := v_department_id = v_owner_department_id;
      v_assignment_role := case when v_is_primary then 'OWNER' else 'SUPPORT' end;

      update public.directive_departments dd
      set
        department_head_id = v_department_head_id,
        is_primary = v_is_primary,
        assignment_role = v_assignment_role
      where dd.directive_id = v_directive_id
        and dd.department_id = v_department_id;

      if not found then
        insert into public.directive_departments (
          id,
          directive_id,
          department_id,
          department_head_id,
      ...
      v_expected_assignment_count := v_expected_assignment_count + 1;
    end loop;
  end loop;

  if v_processed_count <> 77 then
    raise exception
      'CN import blocked: processed row count mismatch. expected=77 actual=%.',
      v_processed_count;
  end if;

  raise notice
    'CN import ready: directives=% expected_assignments=% active_departments=% created_by=% (%).',
    v_processed_count,
    v_expected_assignment_count,
    v_active_department_count,
    v_import_user_id,
    v_import_user_email;
end $$;
