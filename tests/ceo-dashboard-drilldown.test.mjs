...
test("대표 지시사항 API는 부서 배정 테이블에서 시작하고 삭제 지시를 제외한다", () => {
  ...
  assert.match(routeSource, /\.eq\("department_status", query\.status\)/);
  assert.match(routeSource, /buildFallbackResponse/);
  assert.match(routeSource, /listDirectivesForSession/);
  assert.match(routeSource, /assignment\.departmentStatus === query\.status/);
...
test("대표 드릴다운은 검색 파라미터 변경으로 전체 화면을 다시 띄우지 않는다", () => {
  ...
  assert.match(dashboardSource, /lg:grid-cols-\[minmax\(0,1fr\)_minmax\(420px,520px\)\]/);
