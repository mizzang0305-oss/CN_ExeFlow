...
test("비밀번호 재설정 링크는 운영 도메인과 전용 재설정 화면을 사용한다", () => {
  ...
  assert.match(resetClient, /hasRecoveryCredential/);
  assert.match(resetClient, /recoverySessionReady/);
  assert.match(resetClient, /setState\("ready"\)/);
  assert.match(resetClient, /메일 인증이 확인되었습니다/);
  assert.match(resetClient, /메일의 재설정 링크에서만 비밀번호를 변경할 수 있습니다/);
  assert.match(resetClient, /만료된 경우에만 재설정 메일을 다시 요청해주세요/);
