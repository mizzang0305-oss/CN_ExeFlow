...
test("대표 대시보드는 팀장 보고형 요약 패널을 데이터 기반으로 렌더링한다", () => {
  ...
  assert.match(serviceSource, /instructed_at/);
  assert.match(serviceSource, /instructedAt:\s*directive\.instructed_at/);
  assert.doesNotMatch(serviceSource, /instructedAt:\s*directive\.created_at/);
  assert.match(serviceSource, /\.order\("directive_no",\s*\{\s*ascending:\s*true\s*\}\)/);
